...
class CovModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        if (args.model_name == "resnet18") & (args.dataset in ["cifar10", "cifar100", 'comics_crops_bodies']):
            self.backbone = resnet18(zero_init_residual=True)
            self.backbone.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
            self.backbone.maxpool = nn.Identity()
            self.backbone.fc = nn.Identity()
        if (args.model_name == "resnet50") & (args.dataset in ["tiny_imagenet", 'comics_crops_bodies']):
